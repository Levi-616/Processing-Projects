...
HashSet<String> createdPlanetsForColors = new HashSet<String>();
...
void draw() {
  ...
  if (cam.available()) {
          ...
          if (!colorType.equals("NONE")) {
            ...
            if (cluster.size() >= 15 && !colorCentroids.containsKey(colorType)) {
              ...
              if (!colorBots.containsKey(colorType)) {
                colorBots.put(colorType, new Bot(random(width), random(height), getColorFromType(colorType)));
              }
    ...
    for (String color1 : colorBots.keySet()) {
      if (!detectedInFrame.contains(color1)) {
        ...
        createdPlanetsForColors.remove(color1);
      }
    }
    ...
    for (String colorType : colorBots.keySet()) {
      if (colorPresence.getOrDefault(colorType, false)) {
        ...
        if (!createdPlanetsForColors.contains(colorType)) {
          addPlanet(dist(width / 2, height / 2, x, y), 2, 10, b.botColor);
          createdPlanetsForColors.add(colorType);
        }
      }
    }
  }
  ...
  for (int i = 0; i < planets.size(); i++) {
    ...
    for (int j = i + 1; j < planets.size(); j++) {
      ...
      float d1 = dist(p1.pos.x, p1.pos.y, width / 2, height / 2);
      float d2 = dist(p2.pos.x, p2.pos.y, width / 2, height / 2);

      if (d1 < 150 && d2 < 150) {
        float d = dist(p1.pos.x, p1.pos.y, p2.pos.x, p2.pos.y);
        if (d < (p1.radius + p2.radius) / 2) {
          planets.remove(j);
          planets.remove(i);
          i--;
          break;
  ...
  for (int i = planets.size() - 1; i >= 0; i--) {
    ...
    float distToSun = dist(p.pos.x, p.pos.y, width / 2, height / 2);
    if (distToSun < 25) {
      planets.remove(i);
    }
  }
...
class Planet {
  ...
  void update(ArrayList<Planet> others) {
    ...
    float distSq = constrain(force.magSq(), 25, 10000);
    float strength = G * (mass * sunMass) / distSq;
    force.setMag(strength / mass);
    acc.add(force);
    vel.add(acc);
    pos.add(vel);
  }
...
void addPlanet(float d, float v, float m, color c) {
  ...
  vel.mult(v);
  float randomRadius = random(5, 20);
  planets.add(new Planet(pos, vel, m, c, randomRadius, false));
}
